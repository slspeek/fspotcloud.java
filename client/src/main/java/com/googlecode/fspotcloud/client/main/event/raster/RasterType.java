...
public enum RasterType implements ActionDef {
    ADD_COLUMN(
        "add_colum", "Add column", "Adds one column to raster",
        new KeyStroke('C'), null), 
    ADD_ROW(
        "add-row", "Add row", "Adds one row to raster", new KeyStroke('R'), null), 
    REMOVE_COLUMN(
        "remove-column", "Remove column", "Removes one column from the raster",
        new KeyStroke('X'), null), 
    REMOVE_ROW(
        "remove-row", "Remove row", "Removes one row from the raster",
        new KeyStroke('E'), null), 
    SET_DEFAULT_RASTER(
        "reset", "Reset raster", "Resets raster to defaults", new KeyStroke(
            '0'), null), 
    TOGGLE_TABULAR_VIEW(
        "raster", "Toggle raster", "Toggle tabular viewing",
        new KeyStroke(KeyCodes.KEY_SHIFT), new KeyStroke((int)'1')), 
    SET_RASTER_2x2(
        "2x2", "2x2", "Sets the raster to 2 x 2", new KeyStroke('2'), null), 
    SET_RASTER_3x3(
        "3x3", "3x3", "Sets the raster to 3 x 3", new KeyStroke('3'), null), 
    SET_RASTER_4x4(
        "4x4", "4x4", "Sets the raster to 4 x 4", new KeyStroke('4'), null), 
    SET_RASTER_5x5(
