...
public class TagActivity extends AbstractActivity implements TagView.TagPresenter {
	...
	public void goFirst() {
		if (!photoList.isEmpty()) {
			clientFactory.getPlaceController().goTo(new TagPlace(tagId, photoList.get(0)));
		}
	}
	...
	private void requestKeysForTag(final String tagId) {
		clientFactory.getTagService().keysForTag(tagId, new AsyncCallback<List<String>>() {
			
			TagView tagView = clientFactory.getTagView();
			public void onFailure(Throwable caught) {
				tagView.setStatusText("Error recieving photo list for id: " + tagId +".");
			}
