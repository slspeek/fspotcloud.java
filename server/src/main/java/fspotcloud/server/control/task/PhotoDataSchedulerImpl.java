...
public class PhotoDataSchedulerImpl implements PhotoDataScheduler {

	final private PhotoDataScheduler recursiveCall;
	final private SchedulerInterface scheduler;
	final private int MAX_DATA_TICKS;
	
	@Inject
	public PhotoDataSchedulerImpl(@Named("delayed") PhotoDataScheduler recursiveCall,
	...
	@Override
	public void schedulePhotoDataImport(int start, int count) {
		int countWeWillDo = -1;
		...
		if (needToScheduleCount > MAX_DATA_TICKS) {
			...
			recursiveCall.schedulePhotoDataImport(nextStart, nextCount);
